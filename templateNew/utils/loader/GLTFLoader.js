...
/**
 * 来自官方库的迁移，针对GLTFParser.LoadTexture加载ktx2问题进行了修复
 */
...
const mimeTypeToExtendsMap = new Map()
mimeTypeToExtendsMap.set('image/ktx2', '.ktx2')
...
class GLTFParser {
  ...
  loadTexture(textureIndex) {
    ...
    if (sourceDef.uri) {
      ...
      if (handler !== null) loader = handler
    } else if (sourceDef.mimeType) {
      const handler = options.manager.getHandler(mimeTypeToExtendsMap.get(sourceDef.mimeType))
      if (handler !== null) loader = handler
    }
