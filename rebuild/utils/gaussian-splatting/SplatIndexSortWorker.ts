...
class SplatIndexSortWorker {
	...
	onUpdate: ((indices: Uint32Array, count: number) => void ) | null;
	_worker: Worker | null;
	_status: number;
	_indices: Uint32Array | null;
	_lastMVPMatrix: Matrix4;
	...
	init(positions: Float32Array, count: number) {
		...
		this._worker.onmessage = e => {
			if (e.data.init) {
				...
				(this.onUpdate && this._indices ) && this.onUpdate(this._indices, count);
				this._status = WORKER_STATUS.READY;
			}
		};
	}

	update(mvpMatrix: Matrix4) {
...
function workerTemplate(self: Worker) {
	let count = 0;
	let positions: Uint32Array;

	let counts: Uint32Array;
	let starts: Uint32Array;
	let zArray: Float32Array;
	let zIntArray: Int32Array;
